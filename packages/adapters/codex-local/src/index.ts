...
- networkScope (string, optional): "deny" blocks all network egress; "allowlist" permits only networkAllowlist targets through ATV-Teams's HTTP(S) proxy. Off by default.
...
- Fast mode is supported on GPT-5.6 (sol/terra/luna), GPT-5.5, GPT-5.4 and manual model IDs. When enabled for those models, ATV-Teams applies \`service_tier="fast"\` and \`features.fast_mode=true\`.
