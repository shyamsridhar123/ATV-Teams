...
async function createSshAuthArgs(
  ...
  if (config.strictHostKeyChecking) {
    if (config.knownHosts) {
      const knownHosts = await withTempFile("atv-ssh-known-hosts-", config.knownHosts, 0o600);
  ...
  if (config.privateKey) {
    const privateKey = await withTempFile("atv-ssh-key-", config.privateKey, 0o600);
...
async function importGitWorkspaceToSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "atv-ssh-bundle-"));
...
export async function runSshCommand(
  ...
  try {
    ...
    // ATV-Teams sandbox image, so it can expose `node` or an agent CLI only
...
export async function buildSshSpawnTarget(input: {
  ...
  // The SSH target is an operator-configured host, not a ATV-Teams sandbox
...
export async function syncDirectoryFromSsh(input: {
  ...
  const auth = await createSshAuthArgs(input.spec);
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "atv-ssh-sync-back-"));
