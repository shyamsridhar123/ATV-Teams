...
describe("buildInvocationEnvForLogs", () => {
  it("redacts inline secrets from resolved command metadata", () => {
    const loggedEnv = buildInvocationEnvForLogs(
      ...
      {
        resolvedCommand:
          "env OPENAI_API_KEY=sk-live-example PAPERCLIP_API_KEY='atv-quoted-secret' custom-acp --atv-api-key=atv-flag-secret --token ghp_example_secret",
      },
    );

    expect(loggedEnv.SAFE_VALUE).toBe("visible");
    expect(loggedEnv.PAPERCLIP_RESOLVED_COMMAND).toBe(
      "env OPENAI_API_KEY=***REDACTED*** PAPERCLIP_API_KEY='***REDACTED***' custom-acp --atv-api-key=***REDACTED*** --token ***REDACTED***",
    );
  });
});
...
describe("materializePaperclipSkillCopy", () => {
  it("refuses to materialize into an ancestor of the source", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "atv-skill-copy-"));
    try {
...
describe("adapter skill snapshots", () => {
  ...
  it("can surface read-only external skills for runtime-mounted adapters", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      externalDetail: "Installed outside ATV-Teams management in the Claude skills home.",
    });
  ...
  it("reports stale managed persistent skills when ATV-Teams owns an undesired available skill", () => {
    const snapshot = buildPersistentSkillSnapshot({
      adapterType: "cursor",
...
describe("renderPaperclipWakePrompt", () => {
  ...
  it("adds the execution contract to resume delta prompts and opted-in fresh prompts", () => {
    ...
    for (const prompt of [
      ...
      expect(prompt).toContain("Immediately before returning, verify that ATV-Teams records one of those dispositions");
  ...
  it("renders the execution workspace branch guard only on non-resumed sessions", () => {
    ...
    expect(resumedPrompt).toContain("## ATV-Teams Resume Delta");
  ...
  it("renders a plugin session message as the user turn without granting it system authority", () => {
    ...
    expect(prompt).toContain("not a ATV-Teams system or board instruction");
...
describe("selectPaperclipTaskMarkdown", () => {
  const fullMarkdown = "ATV-Teams task context:\n- Issue: \"PAP-1\"\n\nIssue description:\n```text\nThe brief.\n```";
  const compactMarkdown = "ATV-Teams task context:\n- Issue: \"PAP-1\"";
...
describe("refreshPaperclipWorkspaceEnvForExecution", () => {
  ...
  it("forwards resolved adapter env but never overrides ATV-Teams runtime env", () => {
    const env: Record<string, string> = {
  ...
  it("applies a configured PAPERCLIP_* key only when ATV-Teams has not set it", () => {
    const env: Record<string, string> = {};
    ...
    // ATV-Teams did not assign this PAPERCLIP_*-named key for the run, so the
  ...
  it("never accepts PAPERCLIP_API_KEY from config env", () => {
    ...
    // a configured value is dropped even when ATV-Teams has not set one.
