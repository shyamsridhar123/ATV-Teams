      ...
      configSchema: {
        ...
        properties: {
          ...
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Paste your exe.dev API token, or pick a saved ATV-Teams secret. Create one at exe.dev → Settings → API tokens with `/exec` scope (`new`, `ls`, `rm`).",
          },
          sshPrivateKey: {
            type: "string",
            format: "secret-ref",
          ...
          sshIdentityFile: {
            type: "string",
            description:
              "Absolute path to a private key on the ATV-Teams host. Used only when SSH Private Key is empty.",
            "x-paperclip-advanced": true,
            "x-paperclip-group": "SSH access",
          },
          sshPort: {
