  ...
  function buildStrandedIssueRecoveryDescription(input: {
    ...
    return [
      isReviewParticipantRecovery
        ? "ATV-Teams exhausted automatic recovery for a pending execution-review participant and created this explicit recovery task."
  ...
  async function resolveContinuationWaitingOnReview(issue: typeof issues.$inferSelect) {
    ...
    await issuesSvc.addComment(
      ...
      `This task is waiting on ${waitingOn} to finish. ` +
        ...
        "ATV-Teams turned that into a normal dependency wait instead of flagging it as stuck.)",
      {},
      {
        authorType: "system",
      ...
      if (latestRun && adapterFailureClassification) {
        ...
        if (adapterFailureClassification.kind === "provider_quota") {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            recoveryCause: "configuration_incomplete",
            comment:
              "ATV-Teams classified the latest adapter failure as `configuration_incomplete`. " +
        ...
        if (!successfulRunSinceResolution) {
          ...
          if (consecutive >= INTERACTION_CONTINUATION_REQUEUE_MAX_ATTEMPTS && latestPostResolutionRun) {
            ...
            const updated = await escalateStrandedAssignedIssue({
              ...
              comment:
                `ATV-Teams stopped requeueing accepted interaction \`${acceptedContinuationInteraction.id}\` after ` +
      ...
      if (issue.status === "in_review") {
        ...
        if (participantAdapterFailureClassification?.kind === "configuration_incomplete") {
          const updated = await escalateStrandedAssignedIssue({
            ...
            recoveryOwnerAgentId: participantAgentId,
            comment:
              "ATV-Teams classified the active review participant's latest adapter failure as " +
      ...
      if (isUnsuccessfulTerminalIssueRun(latestRun)) {
        ...
        if (classification.kind === "non_retryable") {
          const updated = await escalateStrandedAssignedIssue({
            ...
            notice: {
              body:
                "ATV-Teams detected a non-retryable failure on this issue's continuation run " +
