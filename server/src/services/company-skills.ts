...
const BUILT_IN_SKILL_TEST_RUN_TEMPLATE_BODY = [
  ...
  "",
  "Invoke and use the selected skill under test: `{{skillInvocation}}`. Use the pinned skill revision supplied by ATV-Teams as the source of truth, regardless of any other runtime skills.",
  "",
...
function builtInSkillTestRunTemplate(companyId: string): CompanySkillTestRunTemplate {
  return {
    ...
    description: "ATV-Teams's read-only default harness instructions for Skills Studio runs.",
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but ATV-Teams cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but ATV-Teams cannot find a valid local runtime source for it.`;
}
...
/**
 * A rename target must be a true ATV-Teams-managed local skill: a `local_path`
...
export function companySkillService(db: Db) {
  ...
  async function assertLocalImportSourceAllowed(companyId: string, source: string) {
    ...
    if (!allowed) {
      throw forbidden("Local skill source is outside approved company workspace roots", {
        ...
        remediation: "Import from a configured ATV-Teams workspace or the company managed-skill directory.",
      });
    }
  }
  ...
  async function renameSkill(
    ...
    if (!isPaperclipManagedRenameTarget(skill)) {
      throw unprocessable(
        "Only ATV-Teams-managed skills can be renamed. Catalog, external, project-scanned, and unmanaged local skills are read-only.",
  ...
  async function installFromCatalog(
    ...
    const values = {
      ...
      authorName: storeMetadata.authorName ?? existingByKey?.authorName ?? "ATV-Teams",
