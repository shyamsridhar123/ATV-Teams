...
describe("privateHostnameGuard", () => {
  ...
  it("blocks unknown hostnames with a static remediation command", async () => {
    ...
    expect(res.body?.error).toContain("run node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts allowed-hostname <host>");
  ...
  it("blocks unknown hostnames on page routes with a static plain-text remediation command", async () => {
    ...
    expect(res.send).toHaveBeenCalledWith(
      expect.stringContaining("run node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts allowed-hostname <host>"),
    );
