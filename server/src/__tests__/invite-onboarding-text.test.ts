...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("ATV-Teams Agent Onboarding");
    ...
    expect(text).toContain("Hermes-originated ATV-Teams API usage");
    ...
    expect(text).toContain("Do not rotate or invent a ATV-Teams key manually");
    ...
    expect(text).toContain("Decide which ATV-Teams adapter type matches your runtime.");
  });
