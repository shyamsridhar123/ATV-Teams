...
async function createGitRepo() {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "atv-accepted-plan-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["checkout", "-B", "master"]);
  await runGit(repoRoot, ["config", "user.email", "atv-test@example.com"]);
...
async function createGitRepoWithOrigin() {
  ...
  const originRoot = await mkdtemp(path.join(os.tmpdir(), "atv-accepted-plan-origin-"));
...
describeEmbeddedPostgres("accepted plan workspace refresh", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("atv-accepted-plan-workspace-");
