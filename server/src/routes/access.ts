...
function listAvailableSkills(): AvailableSkill[] {
  ...
  if (paperclipSkillsDir) {
    ...
    } catch { /* skip ATV-Teams skills directory */ }
  }
        ...
        if (apiBaseUrl.protocol !== "http:" && apiBaseUrl.protocol !== "https:") {
          ...
          if (mappedDefaultDashboardRoot) {
            diagnostics.push({
              ...
              hint: "Hermes dashboard and /chat routes are browser UI routes. ATV-Teams gateway calls use /api/health and /api/v1/runs.",
            });
          }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external ATV-Teams agent, save your one-time claim secret, wait for board approval, then claim your ATV-Teams API key through the standard claim endpoint. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. Hermes Gateway agents must use adapterType='hermes_gateway', start a clean Hermes install with API_SERVER_ENABLED=true and a fresh API_SERVER_KEY, then run `hermes gateway run --replace --accept-hooks`. Put the Hermes gateway URL in agentDefaultsPayload.apiBaseUrl, put the exact API_SERVER_KEY value in agentDefaultsPayload.apiKey, and put the reachable ATV-Teams base URL in agentDefaultsPayload.paperclipApiUrl. If you use the default Hermes dashboard root or /chat URL on port 9119, ATV-Teams maps it to /api automatically. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific ATV-Teams skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which ATV-Teams adapter type matches your runtime.

    Use adapterType only when there is a matching ATV-Teams adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    OpenClaw Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "paperclipApiUrl": "https://atv-hostname-your-agent-can-reach:3100",
    ...
    - Set agentDefaultsPayload.apiBaseUrl to the Hermes gateway URL ATV-Teams can reach.
    - Set agentDefaultsPayload.apiKey to the exact same value as API_SERVER_KEY. This is the Hermes gateway key, not the ATV-Teams key.
    ...
    - Watch out: /chat and the dashboard root are browser UI routes. ATV-Teams tests /api/health and starts runs with /api/v1/runs after mapping them to the API base.
    - Set agentDefaultsPayload.paperclipApiUrl to the ATV-Teams base URL Hermes can reach.
    - Use hermes_local when ATV-Teams should start Hermes on the ATV-Teams host.
    - Use hermes_gateway when ATV-Teams should call an already-running Hermes API server.
    - After board approval, claim the ATV-Teams API key once with the claim endpoint below and save it as PAPERCLIP_API_KEY. Store the parsed token field from the raw HTTP JSON response before printing or summarizing it; do not copy token values from chat, transcript, or tool-output previews. A token value containing literal ... or [redacted] is a masked display preview, not a valid key. Do not rotate or invent a ATV-Teams key manually.
    - Hermes-originated ATV-Teams API usage means Hermes calls ATV-Teams with PAPERCLIP_API_URL and PAPERCLIP_API_KEY after approval/key claim. Do not confuse that with agentDefaultsPayload.apiBaseUrl, which points ATV-Teams to Hermes.
    ...
    - Local loopback dashboard root or chat URL: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:9119" or "http://127.0.0.1:9119/chat"; ATV-Teams maps either one to "http://127.0.0.1:9119/api".
    ...
    - Private overlay: use overlay DNS names, for example agentDefaultsPayload.apiBaseUrl = "http://hermes-host.tailnet-name.ts.net:8642" and agentDefaultsPayload.paperclipApiUrl = "http://atv-host.tailnet-name.ts.net:3100".
    - Docker: if ATV-Teams runs in Docker and Hermes runs on the host, use agentDefaultsPayload.apiBaseUrl = "http://host.docker.internal:8642"; if both run in Compose, use the Hermes service name.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set PAPERCLIP_API_KEY and PAPERCLIP_API_URL for future ATV-Teams API calls. The response body includes the full token exactly once, but runtime displays and tool summaries may mask or truncate it. Write the raw response token directly to private storage before logging anything, then verify it with an authenticated ATV-Teams API call. Do not persist displayed previews containing literal ... or [redacted].
    ...
    ## Step 4: Install ATV-Teams skill
