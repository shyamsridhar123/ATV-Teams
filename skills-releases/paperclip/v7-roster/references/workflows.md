...
```bash
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts heartbeat run --agent-id "$PAPERCLIP_AGENT_ID"
```
...
```bash
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```
