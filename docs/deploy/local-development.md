...
```sh
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts worktree:make local-lab --seed-mode minimal
cd ~/paperclip-local-lab
pnpm paperclipai worktree env                       # inspect generated env exports
eval "$(node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts worktree env)"             # bash/zsh
pnpm paperclipai run
...
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts worktree reseed --from . --to paperclip-local-lab
```

When done, shut it down and remove the isolated state explicitly:

```sh
node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts worktree:cleanup local-lab --force
```
