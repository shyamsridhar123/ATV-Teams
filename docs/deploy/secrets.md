...
ATV-Teams stores the feature as value-free metadata plus the user's own secret
value record:
...
identity, not from ATV-Teams secrets.
...
```text
atv-ext/{environment}/{company-id}/user-secrets/{definition-key}/{opaque-owner-id}
```
...
- Keep ATV-Teams-managed AWS values under the ATV-Teams managed namespace.
  External refs under that namespace are blocked by guardrails; use the
  ATV-Teams-managed flow when ATV-Teams should create and rotate the value.

### IAM Caveats

ATV-Teams enforces company scoping, responsible-user derivation, declaration
...
For AWS Secrets Manager, the ATV-Teams runtime role needs `GetSecretValue` and
any required KMS decrypt permission for every user-scoped value it may resolve.
If you link user-specific external refs outside the ATV-Teams managed prefix,
...
refs. ATV-Teams can prevent an agent from choosing another credential owner,
but a runtime role with broad external vault read permissions can still read
what IAM allows if another code path is introduced outside ATV-Teams.
...
- `aws_secrets_manager`: back up ATV-Teams's database for vault metadata
  ...
  same ATV-Teams company at the same AWS namespace and confirming the runtime
