...
or inject external font CSS to make ATV-Teams text render correctly.
...
the process now using its PID. ATV-Teams refuses to create a new request without
...
When ATV-Teams manages embedded PostgreSQL, it suppresses that dependency's eager
`SIGINT`/`SIGTERM` cleanup hooks. ATV-Teams owns signal ordering so the heartbeat
...
>
> The first-run experience often starts with `pnpm paperclipai onboard --yes` (before you have a repo checkout). If your global `~/.npmrc` sets `registry` to a private registry (for example GitHub Packages), `npx` may try to resolve `paperclipai` from that private registry and fail with `E404`.
>
> Diagnostic:
>
> ```sh
...
Use the helper bundled with the ATV-Teams skill from the repo root:

```sh
skills/paperclip/scripts/atv-upload-artifact.sh dist/demo.mp4 \
...
```sh
skills/paperclip/scripts/atv-upload-artifact.sh out/walkthrough.webm \
...
For `codex_local`, ATV-Teams assigns new and updated agents an isolated Codex home under the instance root and blocks shared host/company Codex homes:

- `~/.paperclip/instances/default/companies/<company-id>/agents/<agent-id>/codex-home`

ATV-Teams also persists an empty `OPENAI_API_KEY` override for those agents so a host-level `OPENAI_API_KEY` cannot leak into Codex runs through process inheritance. If an operator explicitly configures `adapterConfig.env.CODEX_HOME`, it must not point at the shared company `codex-home`, `$CODEX_HOME`, or `~/.codex`.
...
When ATV-Teams closes a server-managed git worktree, it also reclaims the isolated instance referenced by that worktree's repo-local `.paperclip/.env`. New server-managed worktrees use a collision-resistant instance id derived from the resolved absolute worktree path, and ATV-Teams persists the resulting instance root as execution-workspace ownership metadata. Cleanup requires the env pointer to match that persisted root, stops a running embedded PostgreSQL process, and then removes the instance directory. The deletion guard only accepts canonical instance paths below `PAPERCLIP_WORKTREES_DIR/instances/`; legacy or mismatched ownership, pointers to the default/live ATV-Teams home, and all other locations are logged and left untouched.
...
Heavier setup that is only needed by a managed runtime service can use `workspaceStrategy.runtimeProvisionCommand`. ATV-Teams runs this command lazily before spawning the first service in a start batch, serializes concurrent provisioning for the same workspace, and records the attempt as `workspace_runtime_provision`. The command receives the same workspace environment as `provisionCommand` and should be idempotent because later service-start batches invoke it again.

## App-Shipped Skills Catalog

The ATV-Teams app ships a curated catalog of company skills out of the box. The
...
paths at request time. Root `skills/` remains reserved for ATV-Teams runtime
...
- defaults to isolated config dir `~/.openclaw-atv-smoke`
