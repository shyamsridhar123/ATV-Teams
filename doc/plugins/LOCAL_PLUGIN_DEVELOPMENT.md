...
paperclipai plugin init @acme/hello-plugin --output ~/dev/atv-plugins

# 3. Install dependencies and start the watch build
cd ~/dev/atv-plugins/hello-plugin
pnpm install
pnpm dev

# 4. In another terminal, install the plugin from its absolute path
paperclipai plugin install ~/dev/atv-plugins/hello-plugin
...
This creates `~/dev/atv-plugins/hello-plugin/` with `src/manifest.ts`, `src/worker.ts`, `src/ui/index.tsx`, an esbuild watch config, a Vitest config, and a snapshot of `@paperclipai/plugin-sdk` from your local ATV-Teams checkout. You can run the package and tests without publishing anything to npm.
...
Before it installs, the CLI probes `GET /api/health` on the instance it is configured to talk to and prints the **target diagnostics** so you can confirm *which* ATV-Teams you are installing into. You will see a confirmation like:

```
Target ATV-Teams: http://127.0.0.1:3100
  health: status=ok  version=0.1.0  mode=local_trusted  exposure=private
Installing plugin from local path: /Users/you/dev/atv-plugins/hello-plugin
✓ Installed acme.hello-plugin v0.1.0 (ready)
Local plugin installs run trusted local code from your machine.
Keep `pnpm dev` running in /Users/you/dev/atv-plugins/hello-plugin;
...
The five-step loop above assumes one ATV-Teams on `http://127.0.0.1:3100`. That breaks down the moment your plugin depends on **server code that only exists on a branch**. Examples:
...
# Target ATV-Teams: http://127.0.0.1:3120
