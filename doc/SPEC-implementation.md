...
- heartbeat finalization evaluates liveness from persisted ATV-Teams state; an issue cannot remain healthy `in_progress` solely because the exiting heartbeat started a local/background watcher
...
ATV-Teams V1 keeps a company-scoped visibility model as the default because centralized authorization and scoped work-object controls are not yet a core V1 control surface.
...
| Company skills | Open to authenticated company agents; core enforces invariants and any stored restriction policy | ATV-Teams EE policy editor, protected-skill controls, presets, simulation, and policy audit UX |
...
Core Skill Studio and ATV-Teams EE MUST treat those codes as hard platform failures, not as prompts to loosen policy.
...
- If ATV-Teams EE is unavailable or removed, core continues to enforce stored policies and expose the policy API. Normal skill work remains available under the open default; explicit denials use core remediation text rather than a broken EE-only link.
...
On a ATV-Teams Cloud-managed instance, `POST /companies` returns `403` with
