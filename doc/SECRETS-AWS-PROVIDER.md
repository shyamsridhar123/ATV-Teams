...
- Hosted provider for ATV-Teams-managed company and user-scoped secrets when ATV-Teams Cloud runs on AWS.
...
When the user-secret service creates ATV-Teams-managed AWS values, keep them
...
ATV-Teams stores the external ref and provider version as metadata. Treat those
...
The same rule applies to user-scoped external refs. ATV-Teams derives the
...
role can read a broad external prefix, that access is broader than ATV-Teams's
...
4. Confirm the ATV-Teams runtime role can call `GetSecretValue` on the restored
