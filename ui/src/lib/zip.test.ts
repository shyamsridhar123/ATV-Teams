...
describe("estimateZipArchiveSize", () => {
  it("matches the real archive byte length for mixed text and binary entries", () => {
    ...
    expect(estimateZipArchiveSize(files, "atv-demo")).toBe(
      createZipArchive(files, "atv-demo").byteLength,
    );
  });
  ...
  it("returns the 22-byte end-of-central-directory record for an empty map", () => {
    expect(estimateZipArchiveSize({}, "atv-demo")).toBe(22);
    expect(estimateZipArchiveSize({}, "atv-demo")).toBe(
      createZipArchive({}, "atv-demo").byteLength,
    );
  });
});
