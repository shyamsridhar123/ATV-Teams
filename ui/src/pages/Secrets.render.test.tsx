...
describe("Secrets page layout", () => {
  ...
  it("writes a new value through the provider for external reference secrets", async () => {
    const externalSecret = makeCompanySecret({
      ...
      name: "atv-cloud/prod/provider/neon/admin-api-key",
      provider: "aws_secrets_manager",
      managedMode: "external_reference",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:atv-cloud/prod/provider/neon/admin-api-key",
      providerConfigId: "vault-aws",
    });
  ...
  it("explains AWS managed secret creation failures with actionable safe details", async () => {
    const rawProviderMessage =
      "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/ATV-Teams is not authorized";
    mockSecretsApi.create.mockRejectedValueOnce(
      new ApiError("AWS Secrets Manager denied the request. Check IAM permissions for this provider vault.", 403, {
        details: {
          ...
          providerConfigId: "vault-aws",
          region: "us-east-1",
          credentialPath: "ATV-Teams server runtime/provider credential path",
          ...
          safeAlternative:
            "If the secret already exists in AWS, link it as an external reference instead of creating a ATV-Teams-managed value.",
        },
      }),
    );
    const root = createRoot(container);
  ...
  it("shows AWS discovery errors without replacing manual vault form values", async () => {
    ...
    mockSecretsApi.providerConfigDiscoveryPreview.mockRejectedValueOnce(
      new ApiError("AWS Secrets Manager denied the request. Check IAM permissions for this provider vault.", 403, {
        details: {
          ...
          actionableMessage:
            "AWS discovery preview needs secretsmanager:ListSecrets in the selected region for the ATV-Teams server runtime/provider credential path.",
          safeAlternative:
    ...
    expect(errorBanner?.textContent).toContain("ATV-Teams server runtime/provider credential path");
