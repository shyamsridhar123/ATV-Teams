---
name: atv-create-plugin
description: >
  Create and develop external ATV-Teams plugins with the CLI-first workflow. Use
  when scaffolding a plugin, iterating on a local plugin, installing it into
  ATV-Teams, or updating plugin authoring docs.
---
...
- Scaffold the plugin into a directory outside the ATV-Teams checkout (e.g. `~/dev/atv-plugins/<name>`).
...
```bash
paperclipai plugin init @acme/my-plugin --output ~/dev/atv-plugins
```
